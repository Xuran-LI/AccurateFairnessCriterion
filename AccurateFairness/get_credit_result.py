...
from utils.util_result import transform_predication, test_model, test_model_acc_fair, test_model_similar, \
    get_data_credit, draw_pie_chart, draw_Fairea_chart, get_average
...
def get_siamese_result_by_model(model_name, protect_name):
    ...
    model_file = "credit_model/{}.h5".format(model_name)
    model = load_model(model_file)

    # 分离原始测试数据的保护属性
    test_file_org = "credit_data/test.txt"
    org_test_data, org_test_label = get_data_credit(test_file_org)
    org_test_pred = transform_predication(predicates=model.predict(org_test_data))
    if protect_name.find("age") >= 0:
        org_none_protect1, org_prot, org_none_protect2 = numpy.split(org_test_data, [20, 21], axis=1)
        test_file_aug = "credit_data/age_test.npz.npy"
    elif protect_name.find("sex") >= 0:
        org_none_protect1, org_prot, org_none_protect2 = numpy.split(org_test_data, [19, 20], axis=1)
        test_file_aug = "credit_data/sex_test.npz.npy"
    elif protect_name.find("multiple") >= 0:
        org_none_protect1, org_prot, org_none_protect2 = numpy.split(org_test_data, [19, 21], axis=1)
        test_file_aug = "credit_data/multiple_test.npz.npy"
    ...
    for j in range(numpy_aug_test_data.shape[1]):
        test_data, label = numpy.split(numpy_aug_test_data[:, j, :], [21, ], axis=1)
        predicate = transform_predication(predicates=model.predict(test_data))
        if protect_name.find("age") >= 0:
            none_protect1, aug_protect, none_protect2 = numpy.split(test_data, [20, 21], axis=1)
        elif protect_name.find("sex") >= 0:
            none_protect1, aug_protect, none_protect2 = numpy.split(test_data, [19, 20], axis=1)
        elif protect_name.find("multiple") >= 0:
            none_protect1, aug_protect, none_protect2 = numpy.split(test_data, [19, 21], axis=1)
    ...
    AF_table.field_names = ["ACC", "Consistency", "FTA", "TFR", "TBR", "FFR", "FBR", "F_R", "F_P", "F_F1"]
...
if __name__ == "__main__":
    print("======================================base line=================================================")
    "base line"
    protected_names = ["sex", "age", "multiple"]
    bl_result = []
