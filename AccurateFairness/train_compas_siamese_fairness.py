...
from tensorflow.python.keras.layers import RandomFourierFeatures
from tensorflow.python.keras.losses import CategoricalCrossentropy, MeanSquaredError, MeanAbsoluteError, Hinge

from utils.util_result import get_data_compas
...
def get_siamese_classification(x, y, epochs, batch_size, protect_n, siamese_size):
    ...
    for j in range(siamese_size):
        input_j = keras.Input(shape=(11,), name="features_{}".format(j))
        output_j = model(input_j)
        input_list.append(input_j)
        output_list.append(output_j)
    # 保存最佳点
    checkpoint = SiameseFair_ModelCheckpoint("compas_model/FS_1_C_{}.h5".format(protect_n),
                                             monitor="performance_num", save_best_only=True)
    checkpoint.set_share_model(model, "compas_model/FS_1_C_{}_share.h5".format(protect_n))
...
def train_siamese_classification(siamese_file, protect_name):
    ...
    train_file = "compas_data/train.txt"
    x_train, y_train = get_data_compas(filename=train_file)
    ...
    for i in range(aug_train.shape[1]):
        x_train, y_train = numpy.split(aug_train[:, i, :], [11, ], axis=1)
        y_train = to_categorical(y_train, num_classes=2)
        train_x_list.append(x_train)
        train_y_list.append(y_train)
...
if __name__ == "__main__":
    "sex"
    sex_file = "compas_data/sex_train.npz.npy"
    train_siamese_classification(sex_file, "sex")
    "race"
    race_file = "compas_data/race_train.npz.npy"
    train_siamese_classification(race_file, "race")
    "age"
    age_file = "compas_data/age_train.npz.npy"
    train_siamese_classification(age_file, "age")
    "multiple"
    multiple_file = "compas_data/multiple_train.npz.npy"
